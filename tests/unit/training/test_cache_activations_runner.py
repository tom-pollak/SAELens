...
def _default_cfg(
    ...
    **kwargs: Any,
    ...
    seqpos_slice = kwargs.get("seqpos_slice")
    if seqpos_slice is not None:
        sliced_context_size = len(range(context_size)[slice(*seqpos_slice)])
    else:
    ...
    assert cfg.total_training_tokens == dataset_num_rows * sliced_context_size
    assert (
