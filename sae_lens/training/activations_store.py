...
class ActivationsStore:
    ...
    def _from_save_activations(
        ...
        act_store_kwargs = cfg.activation_store_kwargs

        normalize_activations = act_store_kwargs.get("normalize_activations", "none")

        streaming = act_store_kwargs.get("streaming", True)
        prepend_bos = act_store_kwargs.get("prepend_bos", True)

        model_kwargs = act_store_kwargs.get("model_kwargs", {})
        autocast_lm = act_store_kwargs.get("autocast_lm", False)

        dataset_trust_remote_code = act_store_kwargs.get(
            "dataset_trust_remote_code", None
        )
        seqpos_slice = act_store_kwargs.get("seqpos_slice", (None,))
    ...
    def from_cache_activations(
        ...
        seqpos_slice = cfg.activation_store_kwargs.get("seqpos_slice", (None,))
        return cls(
            cached_activations_path=cfg.activation_save_path,
            dtype=cfg.dtype,
            hook_name=cfg.hook_name,
            hook_layer=cfg.final_hook_layer,
            context_size=cfg.context_size,
